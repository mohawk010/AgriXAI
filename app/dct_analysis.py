...
def analyse(original_rgb: np.ndarray) -> dict:
    ...
    # The DC peak is exponentially brighter than high frequencies. 
    # By clipping at the 99th percentile, we reveal the dramatic shape of the hidden frequency patterns.
    vmax = np.percentile(log_mag_shifted, 99.0)
    log_mag_norm = np.clip(log_mag_shifted, 0, vmax) / vmax

    # ── Spectrum heatmap ──────────────────────────────────────────────────
    fig, ax = plt.subplots(figsize=(6, 5), facecolor="#0a0f1e")
    ax.set_facecolor("#0a0f1e")
    im = ax.imshow(log_mag_norm, cmap="inferno", interpolation="bilinear")
    ax.set_title("DCT Frequency Spectrum", color="#10b981", fontsize=13, fontweight="bold")
    ax.axis("off")
    ...
    fig2, ax2 = plt.subplots(figsize=(5, 3.2), facecolor="#0a0f1e")
